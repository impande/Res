  ...
  try {
    ...
    if (body.action === 'customize') {
      ...
      const sys = [
        ...
        '- template: "nova" | "sidebar" | "minimal" | "bold" — the base page design. nova = clean cards with a profile banner; sidebar = a sticky left profile rail with content on the right; minimal = a big centered name with borderless, editorial sections; bold = a full-width dark hero with very large type, then light content. Only set this if the user asks to change the overall template/layout style.',
