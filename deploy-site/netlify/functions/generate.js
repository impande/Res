  ...
  try {
    ...
    if (body.action === 'customize') {
      ...
      const sys = [
        ...
        '- topBar: object {text, target} — adds a FIXED, CLICKABLE bar across the very top that scrolls to a section when clicked. target is a section id WITHOUT the # (e.g. "projects"). Use this for requests like "add a CTA/banner at the top that scrolls to projects". Set to null to remove it.',
        '- injectHTML: a string of real HTML added at the top of the content column — use this for custom clickable elements that are NOT a top bar. Do NOT include <script> or on* handlers; they are stripped.',
        ...
        '- For a CTA / banner that scrolls to a section, ALWAYS prefer the topBar field (it renders a guaranteed-correct clickable anchor). Only use injectHTML if a top bar is not what was asked.',
        '- EVERY <a> you put in injectHTML MUST have href="#sectionId" (an existing section id like #projects) for in-page scrolling, or a full https:// URL for an external link. NEVER use href="#", href="/", href="" or any relative path — those navigate away from the page.',
        '',
